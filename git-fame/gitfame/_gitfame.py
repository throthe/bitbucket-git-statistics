...
Options:
  ...
  --file=<file>  File used to write the output (default: None).    
  ...
  --show-total      Show the total stats in the output [default: False].
...
def tabulate(
    ...
    tab = [
        [
            auth,
            s["loc"],
            s.get("insertations", 0),
            s.get("deletions", 0),
            s.get("commits", 0),
            len(s.get("files", [])),
            round(100 * float(s["loc"] / max(1, stats_tot["loc"])), 2),
            round(100 * s.get("commits", 0) / max(1, stats_tot["commits"]), 2),
            round(100 * len(s.get("files", [])) / max(1, stats_tot["files"]), 2),
        ]
        for (auth, s) in it_as()
    ]
    if cost:
        ...
        if cost & COST_MONTHS:
            ...
            tab = [i[:1] + [round(3.2 * (i[1] / 1e3) ** 1.05, 2)] + i[1:] for i in tab]
            stats_tot.setdefault("months", "%.1f" % sum(i[1] for i in tab))
        if cost & COST_HOURS:
            COL_NAMES.insert(1, "hrs")
            tab = [
                i[:1] + [round(hours(auth_stats[i[0]]["ctimes"]), 2)] + i[1:]
                for i in tab
            ]
    ...
    if backend in ["yaml", "yml", "json", "csv", "tsv"]:
        ...
        COL_LENS = [
            max(len(Str(i[j])) for i in [COL_NAMES] + tab)
        ...
        #     tab, COL_NAMES, tablefmt=backend, floatfmt=".2f"
        # )
        ...
        if show_total:
            ...
            return tabber.tabulate(tab, COL_NAMES, tablefmt=backend, floatfmt=".2f")
...
def _get_auth_stats(
    ...
    file_list = (
        check_output(git_cmd + ["ls-files", "--with-tree", branch]).strip().split("\n")
    )
    ...
    def stats_append(fname, auth, loc, tstamp, inss=0, dels=0):
        auth = str(auth)
        tstamp = int(tstamp)
        try:
            auth_stats[auth]["loc"] += loc
            auth_stats[auth]["insertations"] += inss
            auth_stats[auth]["deletions"] += dels
        except KeyError:
            auth_stats[auth] = {
                "loc": loc,
                "insertations": inss,
    ...
    if churn & CHURN_SLOC:
        for fname in tqdm(
            file_list,
            desc=gitdir if prefix_gitdir else "Processing",
            ...
            for loc, auth, tstamp in RE_AUTHS_BLAME.findall(
                blame_out
            ):  # for each chunk
                loc = int(loc)
        ...
        for auth, tstamp, fnames in blame_out:
            ...
            for i in fnames.strip().split("\n"):
                try:
                    ...
                    inss = int(inss)
                    dels = int(dels)
                    loc = (inss if churn & CHURN_INS and inss else 0) + (
                        dels if churn & CHURN_DEL and dels else 0
                    )
                    stats_append(fname, auth, loc, tstamp, inss, dels)
    ...
    if show_email:  # replace author name with email
        ...
        for auth, stats in getattr(old, "iteritems", old.items)():
            ...
            i["loc"] += stats["loc"]
            i["insertations"] += stats["insertations"]
            i["deletions"] += stats["deletions"]
...
def run(args):
    ...
    gitdirs = [
        path.join(*path.split(i)[:-1]) if path.split(i)[-1] == ".git" else i
    ...
    if args.recurse:
        ...
        while i < nDirs:
            if path.isdir(gitdirs[i]):
                for root, dirs, fns in tqdm(
                    os.walk(gitdirs[i]),
                    desc="Recursing",
                    unit="dir",
                    disable=args.silent_progress,
                    leave=False,
                ):
                    if ".git" in fns + dirs:
    ...
    if churn & (CHURN_INS | CHURN_DEL) and args.excl:
        log.warning(
            "--loc=ins,del includes historical files"
    ...
    statter = partial(
        ...
        M=args.M,
        C=args.C,
        warn_binary=args.warn_binary,
        bytype=args.bytype,
        show_email=args.show_email,
        prefix_gitdir=len(gitdirs) > 1,
        churn=churn,
    ...
    for k in stats_tot:
        stats_tot[k] = sum(
            int_cast_or_len(stats.get(k, 0)) for stats in auth_stats.values()
        )
    log.debug(stats_tot)
