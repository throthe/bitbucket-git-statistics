...
def print_unicode_with_file(msg, end="\n", err="?", file=None):
    """Print `msg`, replacing Unicode characters with `err` upon failure.

    Parameters:
    - msg (str): The message to be printed or written.
    - end (str): The string appended at the end (default is newline).
    - err (str): The replacement character for Unicode encode errors (default is '?').
    - file (str or None): The filename to write to. If None, prints to stdout.
    """
    output_stream = (
        open(file, "w", encoding="utf-8") if file is not None else sys.stdout
    )
